...
Since the types are autogenerated from [redis-doc](https://github.com/redis/redis-doc), which adhere to a formal schema, some of the types might be incomplete, or in some cases, wrong. The type errors are easy enough to fix, but not easy to find, so if you think you've come across a case like this, please [raise an issue](https://github.com/mmkal/handy-redis/issues).
