...
const simplify = (func: (...args: any[]) => string | number | boolean | null | undefined) => (...args: any[]) =>
    `${func.name} => ${func(...args)}`;

export const _isFinite = simplify(isFinite);
export const _typeOf = simplify(function typeOf(thing) {
    return Array.isArray(thing) ? "array" : typeof thing;
});
export const _sorted = simplify(function sorted(arr: any[]) {
    return inspect(arr.sort());
});
...
export const _arrayLength = simplify(function arrayLength(x: any[]) {
    return x.length;
});
const noNumberValues = (o: any) => (/^\d+$/.test(o) ? "[a number]" : o);
const sortArrays = (o: any) => (Array.isArray(o) ? _sorted(o) : o);
const ignoreDecimals = (o: any) =>
    JSON.parse(JSON.stringify(o).replace(/(\d+)\.(\d+)/g, number => number.split(".")[0] + ".??"));
const ignoreStreamIds = (streamIds: string[]) => (o: any) =>
    JSON.parse(
...
addOverride("/(sunionstore|sadd|smembers|sunion|smove|srem|spop|sinterstore|sdiffstore|sdiff).md", outputs =>
    outputs.map(sortArrays)
);
